...
// Resilience mode — "what's actually usable offline" summary, for the
// cache panel in ConnectionBanner. Everything here lives in this one
// browser's localStorage (see localCache.js) — never sent anywhere, never
// shared between users or devices, gone if the user clears site data.
// One entry per lane (origin+destination+commodity), most-recently-saved
// first, so the panel can tell someone which routes will actually work
// before they try one that isn't cached.
export function getCacheSummary() {
  ...
  for (const e of forecasts) {
    const key = `${e.origin_port}|${e.destination_port}|${e.commodity}`;
    const existing = laneMap.get(key);
    if (!existing || existing.savedAt < e.savedAt) {
      laneMap.set(key, {
        origin: e.origin_port,
        destination: e.destination_port,
        commodity: e.commodity,
        savedAt: e.savedAt,
      });
    }
  }
  const lanes = Array.from(laneMap.values()).sort((a, b) => b.savedAt - a.savedAt);
  ...
  return {
    meta: meta?.data
      ? {
          ...
          originCount: meta.data.origins?.length || 0,
          destinationCount: meta.data.destinations?.length || 0,
          commodityCount: meta.data.commodities?.length || 0,
        }
      : { available: false },
    vessels: vessels?.data
      ? { available: true, savedAt: vessels.savedAt, count: vessels.data.length }
      : { available: false },
    forecasts: {
      count: forecasts.length,
      lanes,
      latestSavedAt: forecasts.reduce((max, e) => Math.max(max, e.savedAt || 0), 0) || null,
    },
  };
}
