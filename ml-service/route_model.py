...
def train(output_dir: str | Path | None = None):
    ...
    meta={
        ...
        "direct_ais_status":"not supplied in the uploaded project; PortWatch is the current AIS-derived fallback.",
    }
...
class RouteModel:
    ...
    def _row(self, origin, destination, when):
        ...
        return pd.DataFrame([row]), source_month

    def predict(self, origin, destination, shipment_date, current_spot_rate_usd_per_ton=None):
        row, source_month=self._row(origin,destination,shipment_date)
        current_proxy=float(row["bdry_proxy"])
        if not self.bdry.empty:
        ...
        for h,model in self.models.items():
            ...
            item={"horizon_months":h,"predicted_market_proxy":round(pred,4),
                  ...
                  "ais_feature_stale":bool(pd.Timestamp(shipment_date)>pd.Timestamp(self.meta["latest_route_feature_month"]))}
