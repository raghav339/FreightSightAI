...
def optimize(req, route_model: RouteModel, port_utils):
    total=float(req.total_program_tons or req.cargo_weight_tons)
    ...
    return {
        ...
        "market_forecast":rf,
        "methodology":"Enumerates both-port-feasible vessel classes, computes COA voyage count from dataset-derived typical DWT, uses direct observed-route forecasts when available or an AIS-enhanced market proxy calibrated to a user-supplied spot benchmark otherwise, and adds a transparent risk buffer. It is not a broker/charter quote.",
    }